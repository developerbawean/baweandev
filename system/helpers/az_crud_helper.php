...
defined('BASEPATH') OR exit('No direct script access allowed');   
    ...
    if(!function_exists('az_crud_edit')) {
        function az_crud_edit($data = '', $return = false, $where = '', $table = '') {
            ...
            $ci->db->select($select);
            $ci->db->where($where."id".$ci_table, $id);

            $rdata = $ci->db->get($ci_table)->result_array();
    ...
    if(!function_exists('az_crud_save_api')) {
        function az_crud_save_api($idpost, $table, $data) {
            $ci =& get_instance();           
    ...
    if(!function_exists('az_crud_update')) {
        function az_crud_update($idpost, $table, $data, $where) {
            $ci =& get_instance();           
    ...
    if (!function_exists('az_crud_upsert')) {
        function az_crud_upsert($idpost, $table, $data, $where = null) {
            $ci =& get_instance();
    ...
    if(!function_exists('az_crud_datet')) {
        function az_crud_datet($date, $type = 'j') {
            if (varlen($date) == 0) {
                return NULL;
            }
            else {
    ...
    if(!function_exists('az_crud_number')) {
        function az_crud_number($number) {
            ...
            $is_decimal_dot = az_get_config('is_decimal_dot', 'config_app');
            if($is_decimal_dot) {
    ...
    if (!function_exists('get_time_micro')) {
        function get_time_micro() {
            $t = microtime(true);
            $micro = sprintf("%06d", ($t - floor($t)) * 1000000);
            $timestamp = (int) $t; // ✅ Ubah ke int secara eksplisit
