...
defined('BASEPATH') OR exit('No direct script access allowed');   
    ...
    if(!function_exists('az_thousand_separator_decimal')){
        function az_thousand_separator_decimal($key ='', $decimal = 2) {
            ...
            $is_decimal_dot = az_get_config('is_decimal_dot', 'config_app');
            if($is_decimal_dot) {
                $data = number_format($key, $decimal, ".", ",");
                ...
                if (count($parts) === 1) {
                    ...
                    $decimalPart = rtrim(rtrim($parts[1], '0'), '.');
                    if ($decimalPart == '') {
                        return $parts[0];
                    }
    ...
    if(!function_exists('az_thousand_separator_decimal_dot')){
        function az_thousand_separator_decimal_dot($key ='') {
            $data = number_format($key, 4, ".", ",");          
            return $data;
        }
    }
    ...
    if (!function_exists('az_terbilang_koma')) {
        function az_terbilang_koma($x) {
    ...
    if(!function_exists('az_css')){
        function az_css() {
            ...
            $end_css = $ci->config->item('end_css');
            ...
            foreach((array) $end_css AS $item){
                $str .= '<link rel="stylesheet" href="'.base_url().'assets/plugins/'.$item.'" type="text/css" />'."\n";
            }

            if (is_file(APPPATH.'assets/plugins/az_theme/az_theme_custom.css')) {
                $str .= '<link rel="stylesheet" href="'.base_url().AZAPP.'assets/plugins/az_theme/az_theme_custom.css?v'.Date('YmdHis').'" type="text/css" />'."\n";   
            }
